...
export function DashboardPage() {
  const { setActiveModule, setFinanceSubView, setTransportSubView, isOnline, pendingCount, dashboardWidgets } = useAppStore();
  ...
  const renderWidget = (widgetId: string) => {
    switch (widgetId) {
      case "stats":
        return (
          <motion.div key="stats" variants={itemVariants} className="grid grid-cols-3 gap-3">
            <Card className="border-0 shadow-sm rounded-2xl cursor-pointer hover:shadow-md transition-shadow" onClick={() => { setActiveModule("finance"); setFinanceSubView("budgets"); }}>
              <CardContent className="p-3 text-center">
                <Receipt className="size-5 text-amber-500 mx-auto mb-1" />
                <p className="text-lg font-bold text-gray-900 dark:text-gray-100">{budgetPercentage}%</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">Presupuesto</p>
              </CardContent>
            </Card>
            <Card className="border-0 shadow-sm rounded-2xl cursor-pointer hover:shadow-md transition-shadow" onClick={() => { setActiveModule("finance"); setFinanceSubView("debts"); }}>
              <CardContent className="p-3 text-center">
                <CreditCard className="size-5 text-rose-500 mx-auto mb-1" />
                <p className="text-lg font-bold text-gray-900 dark:text-gray-100">{debtPercentage}%</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">Deudas</p>
              </CardContent>
            </Card>
            <Card className="border-0 shadow-sm rounded-2xl cursor-pointer hover:shadow-md transition-shadow" onClick={() => { setActiveModule("finance"); setFinanceSubView("savings"); }}>
              <CardContent className="p-3 text-center">
                <PiggyBank className="size-5 text-emerald-500 mx-auto mb-1" />
                <p className="text-lg font-bold text-gray-900 dark:text-gray-100">{savingsPercentage}%</p>
      ...
      case "transport":
        ...
        return (
          <motion.div key="transport" variants={itemVariants}>
            <Card className={`border-0 shadow-md rounded-2xl cursor-pointer hover:shadow-lg transition-all overflow-hidden ${hasLowFuel ? "ring-2 ring-red-400 dark:ring-red-600" : ""}`} onClick={() => { setActiveModule("transport"); setTransportSubView("vehicles"); }}>
              <div className={`bg-gradient-to-r ${hasLowFuel ? "from-red-500 to-orange-500" : "from-cyan-600 to-blue-600"} p-4 relative overflow-hidden`}>
                ...
                <div className="relative z-10 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {primaryVehicle.plate && (
                        <span className="rounded bg-white/25 px-1.5 py-0.5 text-[11px] font-semibold uppercase tracking-normal text-white">{primaryVehicle.plate}</span>
                      )}
                      {hasLowFuel && (
                        <span className="text-[11px] bg-white/30 text-white rounded-full px-1.5 py-0.5 animate-pulse">Bajo</span>
                      )}
                    </div>
                    <div className="flex items-center gap-3 mt-1">
                      <p className="text-xl font-bold text-white">{primaryVehicle.currentFuel?.toFixed(1) ?? "0"}<span className="text-xs font-normal text-white/60 ml-0.5">gal</span></p>
                  ...
                  <div className="flex-shrink-0 text-right">
                    <p className="text-2xl font-bold text-white">{Math.round(primaryVehicle.fuelLevel ?? 0)}%</p>
                    <div className="w-16 h-1.5 bg-white/20 rounded-full mt-1.5 overflow-hidden">
                      <div className="h-full bg-white/80 rounded-full transition-all" style={{ width: `${Math.min(primaryVehicle.fuelLevel ?? 0, 100)}%` }} />
                    </div>
                  </div>
                </div>
              </div>
      ...
      case "water":
        return (
          <motion.div key="water" variants={itemVariants}>
            <WaterWidget />
          </motion.div>
        );
      case "finance":
        return (
          <motion.div key="finance" variants={itemVariants}>
            <Card className="border-0 shadow-md rounded-2xl cursor-pointer hover:shadow-lg transition-all overflow-hidden" onClick={goToFinanceOverview}>
              <CardContent className="p-5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Ve tu resumen financiero completo con gráficos y proyecciones</p>
                  </div>
      ...
      case "aura":
        return (
          <motion.div key="aura" variants={itemVariants}>
            <Card className="border-0 shadow-md rounded-2xl hover:shadow-lg transition-all overflow-hidden">
              <CardContent className="p-5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Aura</h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">IA integrada de Quid. Por ahora puedes vincularla desde ajustes.</p>
                  </div>
                  <div className="flex shrink-0 flex-col gap-1.5">
                    <Button size="sm" className="h-8 rounded-xl bg-gradient-to-r from-violet-600 to-sky-500 px-3 text-xs text-white" onClick={() => window.open("https://t.me/Aura_RQC_Bot", "_blank", "noopener,noreferrer")}>
                      Abrir Aura
                    </Button>
                    <Button size="sm" variant="ghost" className="h-7 rounded-lg px-2 text-[11px] text-gray-500" onClick={() => setActiveModule("settings")}>
                      Configurar
                    </Button>
                  </div>
                </div>
      ...
      case "actions":
        return (
          <motion.div key="actions" variants={itemVariants}>
            <div className="flex gap-2">
              <Button variant="outline" className="flex-1 h-12 rounded-xl border-dashed border-emerald-300 dark:border-emerald-700 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 hover:border-emerald-400" onClick={() => { setActiveModule("finance"); setFinanceSubView("accounts"); }}>
                <ArrowUpRight className="size-4 mr-1" />
                Ingreso
              </Button>
              <Button variant="outline" className="flex-1 h-12 rounded-xl border-dashed border-rose-300 dark:border-rose-700 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/20 hover:border-rose-400" onClick={() => { setActiveModule("finance"); setFinanceSubView("accounts"); }}>
                <ArrowDownRight className="size-4 mr-1" />
                Gasto
              </Button>
              <Button variant="outline" className="flex-1 h-12 rounded-xl border-dashed border-amber-300 dark:border-amber-700 text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:border-amber-400" onClick={() => { setActiveModule("finance"); setFinanceSubView("budgets"); }}>
    ...
    >
      ...
      {dashboardSection === "summary" ? (
        <div className="space-y-4">
          {/* BALANCE CARD IS ALWAYS AT THE TOP */}
          <motion.div variants={itemVariants}>
